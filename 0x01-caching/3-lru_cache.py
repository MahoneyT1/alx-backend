...
class LRUCache(BaseCaching):
    ...
    def __init__(self):
        """initilizes the parents class by call """
        super().__init__()
    ...
    def put(self, key, item):
        ...
        if key is not None and item is not None:
            ...
            if key not in self.recently_used_cache:
                self.recently_used_cache.append(key)

        if len(self.cache_data) > BaseCaching.MAX_ITEMS:
            popp = self.recently_used_cache.pop(0)
            del self.cache_data[popp]
